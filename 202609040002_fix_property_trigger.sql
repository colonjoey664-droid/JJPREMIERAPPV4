-- Fixes properties_enforce_company from 202609040001.
-- Properties has no service_id column, so it needs its own trigger function.
...
create trigger properties_enforce_company
  before insert or update on public.properties
  for each row execute function public.enforce_property_company();

drop trigger if exists customer_services_enforce_company on public.customer_services;
create trigger customer_services_enforce_company
  before insert or update on public.customer_services
  for each row execute function public.enforce_customer_service_company();
