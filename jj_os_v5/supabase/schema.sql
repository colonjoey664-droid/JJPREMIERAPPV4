...
create table public.customers (id uuid primary key default gen_random_uuid(), company_id uuid not null references public.companies(id) on delete cascade, name text not null, phone text, email text, status text not null default 'lead', service_type text, notes text, lifetime_paid numeric(12,2) not null default 0, balance_owed numeric(12,2) not null default 0, created_at timestamptz not null default now());
